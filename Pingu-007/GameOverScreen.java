...
/** Game-over overlay composition and visuals. */
public final class GameOverScreen extends AbstractMenuScreen {
    ...
    public GameOverScreen(SoundManager ignoredSoundManager) {
        entries = List.of(
                new ButtonMenuDefinition("RENASCER NO CHECKPOINT", BTN_W, BTN_H,
                        MenuAction.navigateTo(GameState.PLAYING)).createEntry(),
    ...
    protected GameState updateScreen(MenuContext context) {
        return controller.update(context, GameState.GAME_OVER);
    }
    ...
    protected void drawScreen(Graphics2D graphics, MenuViewport viewport) {
        ...
        MenuPainter.drawCenteredTextInWidth(graphics, "GAME OVER", pixelFont, viewport.width(), panel.y + 55,
                Color.RED, new Color(0, 0, 0, 180), 2, 2);
